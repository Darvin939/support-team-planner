...
## Production-миграция SQLite

Перед запуском новой версии остановите все экземпляры backend, выполняющие запись, и создайте
WAL-безопасную резервную копию через SQLite backup API. Обычное копирование `database.db` недостаточно:
актуальные изменения могут находиться в WAL-файле.

Примените миграции и полную проверку целостности отдельной командой:

```bash
python migrate_database.py --database database.db
```

Команда применяет те же миграции, что и startup backend, проверяет схему, журнал миграций,
внешние ключи и `PRAGMA quick_check`. Backend следует запускать только после успешного завершения.
При ошибке текущий шаг откатывается без повышения `user_version`.

Для отката после успешного развёртывания остановите backend и восстановите проверенную резервную
копию через SQLite backup API; не копируйте файл поверх работающей WAL-базы.
