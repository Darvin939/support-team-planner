...
1. Перед тестом создать резервную копию `database.db` с уникальным именем. Не перезаписывать существующие копии:

   ```powershell
   $stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
   Copy-Item -LiteralPath 'database.db' -Destination "database.db.ui-test-$stamp.bak"
   ```

2. Собрать production-frontend. Команду запускать из `frontend`:
...
4. Если тест изменял БД, после теста вернуть `database.db` из созданной копии. Перед восстановлением обязательно остановить backend, чтобы SQLite-соединения были закрыты. Не удалять backup до успешной проверки восстановленной БД.
...
- После submit формы ждать не тольо URL, но и целевой DOM-элемент защищённой страницы.
