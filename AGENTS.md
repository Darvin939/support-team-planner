...
- Все изменения Python-кода должны сохранять совместимость с Python 3.9. Не использовать синтаксис и API, появившиеся в Python 3.10 и новее; в частности, для объединений типов использовать `Optional` и `Union` из `typing`, а не оператор `|`.

## Рабочий процесс OpenSpec

- Первым делом запусти OpenSpec Explore чтобы все изучить (если это необходимо)
- Далее задачу прогони через Grilling. Как только все ответы будут получены и уточнены, можно будет запускать OpenSpec
- И только после OpenSpec: подготовить или обновить артефакты изменения, реализовать задачи, синхронизировать основные спецификации и архивировать завершённое изменение.
...
- Логику, состояние и взаимодействия frontend-компонентов проверять unit/component-тестами на Vitest + React Testing Library.
- Чистые функции и hooks проверять Vitest-тестами без браузера, если DOM не требуется.
- Не требовать отдельный test-файл для каждого `.tsx`: компонент может быть покрыт тестом родительского компонента, если у него нет самостоятельного поведения. Для компонентов и hooks с собственной логикой должен существовать собственный тест либо явная запись в матрице покрытия о родительском тесте.
- Не использовать snapshot-тесты как единственную проверку поведения.
- Python + Playwright использовать для визуальных, responsive и сквозных production-сценариев: layout, sticky/fixed элементы, переполнение, модальные окна, навигация и интеграция с реальным backend.
...
- Текст пункта Ant Design Menu может совпадать с заголовком страницы и давать strict-mode violation. Для перехода выбирать видимый `.ant-menu-item:visible` с полным фактическим названием пункта, а готовность страницы проверять отдельно через `get_by_role('heading', ...)`.
- Для маршрутов с восстановлением сохранённой команды не считать промежуточный `/planning` или `/journal` конечным состоянием: ждать URL с конкретным `teamId`, иначе wildcard `**/journal**` может завершиться до redirect.
- При последовательном выборе значений в Ant Design Select анимация закрывающегося dropdown может перехватывать следующий pointer-click. Для подписанных полей предпочитать `get_by_label(...)` и выбор клавишами `ArrowDown`/`Enter`.
